...
from utils.custom import turn_off_GPU
import datetime
...
def main():
    ...
    n_obj_model_dirs = []
    for root, dirs, files in os.walk('./logging/offlineILPnPThreeExp/GODICE_semi_0.50_9'):
        for name in dirs:
            if 'run' in name:
    ...
    for one_obj_dir in one_obj_model_dirs:
        for n_obj_dir in n_obj_model_dirs:
            ...
            avg_return, std_dev_return = agent.compute_avg_return(eval_demos=100, avg_of_reward=True)
