...
from utils.custom import state_to_goal, repurpose_skill_seq, turn_off_GPU
...
def evaluate(algo: str, num_eval_demos=100, eval_with_expert_assist:bool = False):
    ...
    # List Model Directories
    model_dirs = []
    for root, dirs, files in os.walk('./logging/offlineILPnPThreeExp/GODICE_none_0.50_9'):
        for name in dirs:
            if 'run' in name:
                model_dirs.append(os.path.join(root, name, 'models'))
...
if __name__ == "__main__":
    ...
    record(algo='DemoDICE', root_dir='./logging/offlineILPnPThreeExp/gDemoDICE_0.50')

    # num_runs = 5
    # for i in range(num_runs):
    #     run(debug=False, algo='BC')
