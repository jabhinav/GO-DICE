...
class DemoDICE(tf.keras.Model, ABC):
	def __init__(self, args: Namespace):
		...
		if args.two_object:
			self.num_skills = 5 if args.wrap_skill_id == '2' else 6 if args.wrap_skill_id == '1' else 3
		else:
			self.num_skills = 3
	...
	def get_init_skill(self):
		if self.args.wrap_skill_id == '0':
			skill = tf.one_hot(0, self.num_skills, dtype=tf.float32)  # Object 0 first
...
class Agent(AgentBase):
	def __init__(self, args,
		...
		super().__init__(args, DemoDICE(args), 'demoDICE', expert_buffer_unseg, policy_buffer_unseg)
