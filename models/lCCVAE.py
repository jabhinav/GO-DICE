...
from her.transitions import sample_no_her_all_transitions, sample_c_aware_transitions
...
class lCCVAE(tf.keras.Model):
	def __init__(self, args, norm_s: Normalizer, norm_g: Normalizer):
		super(lCCVAE, self).__init__()
		self.args = args
		self.encoder = Encoder(args.z_dim)
		self.decoder = Policy(args.a_dim, args.action_max)
	...
	def classifier_loss(self, data, k=100):
		[post_locs, post_scales] = self.encoder(data['states'], data['goals'], data['prev_latent_modes'])
	...
	def compute_sup_loss(self, data):
		...
		# GENERATION: Compute the log-likelihood of actions i.e. p(action|s, g, z)
		actions_mu = self.decoder(data['states'], data['goals'], z_g)  # Does using z (instead of y) here help?
		...
		ll = -tf.reduce_sum(tf.math.squared_difference(data['actions'], actions_mu), axis=-1)
		kl = kl_divergence_gaussian(
			mu1=post_locs, log_sigma_sq1=tf.math.log(post_scales + self.args.underflow_eps),
			mu2=prior_locs, log_sigma_sq2=tf.math.log(prior_scales + self.args.underflow_eps), mean_batch=False
		)
		
		elbo = tf.math.multiply(w, ll - kl - log_qy_z) + log_py + self.args.alpha * log_qy_x
		loss_g = tf.reduce_mean(-elbo)
		
		return loss_g, tf.reduce_mean(-ll), tf.reduce_mean(kl), tf.reduce_mean(-log_qy_x)
	...
	def do_eval(self, data):
		...
		actions_mu = self.decoder(data['states'], data['goals'], z_g)
	...
	def train(self, data):
		with tf.GradientTape() as tape:
			total_loss, ll, kl_div, class_loss = self.compute_sup_loss(data)
		
		gradients = tape.gradient(total_loss, self.trainable_variables)
		self.optimiser.apply_gradients(zip(gradients, self.trainable_variables))
		return total_loss, ll, kl_div, class_loss
	
	def act(self, state, achieved_goal, goal, prev_latent_mode, compute_c=False, **kwargs):
		...
		[post_locs, post_scales] = self.encoder(state, goal, prev_latent_mode)
		z_g = self.sample_normal(post_locs, post_scales, self.args.z_dim)
		
		action_mu = self.decoder(state, goal, z_g)
		
		# Add layer - Skip Connection
		action = tf.clip_by_value(action_mu, -self.args.action_max, self.args.action_max)
		if compute_c:
			latent_mode = self.classifier(z_g)
			# Convert latent mode to one-hot vector
			latent_mode = tf.one_hot(tf.argmax(latent_mode, axis=-1),  tf.shape(latent_mode)[-1])
			return action, latent_mode
		return action
...
class Agent(object):
	def __init__(self, args, expert_buffer: ReplayBufferTf, val_buffer: ReplayBufferTf):
		...
		# Define the Buffers
		self.transition_fn = sample_c_aware_transitions
		...
		# Declare Model
		self.model = lCCVAE(args, self.norm_s, self.norm_g)
		
		# Evaluation
		self.eval_env: MyPnPEnvWrapperForGoalGAIL = get_PnP_env(args)
		self.eval_rollout_worker = RolloutWorker(
			self.eval_env, self.model, T=args.horizon, rollout_terminate=args.rollout_terminate,
			exploit=True, noise_eps=0., random_eps=0., compute_c=True, use_target_net=False, render=False
		)
	...
	def load_model(self, dir_param):
		...
		# BUILD First
		_ = self.model.encoder(np.ones([1, self.args.s_dim]), np.ones([1, self.args.g_dim]), np.ones([1, self.args.c_dim]))
		_ = self.model.decoder(np.ones([1, self.args.s_dim]), np.ones([1, self.args.g_dim]), np.ones([1, self.args.z_dim]))
	...
	def process_data(self, transitions, expert=False, is_supervised=False):
		...
		transitions_c = transitions.copy()
		states, achieved_goals, goals = transitions_c['states'], transitions_c['achieved_goals'], transitions_c['goals']
		...
		if self.args.use_norm:
			...
			transitions_c['states'] = self.norm_s.normalize(states)
			transitions_c['achieved_goals'] = self.norm_g.normalize(achieved_goals)
			transitions_c['goals'] = self.norm_g.normalize(goals)
		else:
			transitions_c['states'] = states
			transitions_c['achieved_goals'] = achieved_goals
			transitions_c['goals'] = goals
		
		# Define if the transitions are from expert or not/are supervised or not
		transitions_c['is_demo'] = tf.cast(expert, dtype=tf.int32) * tf.ones_like(transitions_c['successes'], dtype=tf.int32)
		transitions_c['is_sup'] = tf.cast(is_supervised, dtype=tf.int32) * tf.ones_like(
			transitions_c['successes'], dtype=tf.int32)
		
		# Make sure the data is of type tf.float32
		for key in transitions_c.keys():
			transitions_c[key] = tf.cast(transitions_c[key], dtype=tf.float32)
		
		return transitions_c
	...
	def sample_data(self, expert_batch_size, batch_size=None):
		...
		processed_transitions = self.process_data(
			expert_transitions, expert=tf.constant(True, dtype=tf.bool), is_supervised=tf.constant(True, dtype=tf.bool)
		)
		...
		with tqdm(total=args.num_epochs, position=0, leave=True, desc='Training: ') as pbar:
			...
			for epoch in range(args.num_epochs):
				...
				for _ in range(args.n_batches):
					...
					total_loss, ll, kl_div, class_loss = train_losses
					
					# Log the VAE Losses
					with self.summary_writer.as_default():
						tf.summary.scalar('train/loss/total', total_loss, step=global_step)
						tf.summary.scalar('train/loss/vae/ll', ll, step=global_step)
				...
				if args.do_eval:
					...
					val_class_loss, val_ll = self.model.do_eval(val_trans)
					
					# Log Validation Losses
					with self.summary_writer.as_default():
						tf.summary.scalar('val/loss/vae/classifier', val_class_loss, step=epoch)
						tf.summary.scalar('val/loss/policy', val_ll, step=epoch)
					...
					if (epoch + 1) % args.log_interval == 0:
						...
						for n in range(args.eval_demos):
							...
							plot_metrics(
								metrics=[episode['distances'].numpy()[0]],
								labels=['|G_env - AG_curr|'],
					...
					if val_ll.numpy() < monitor_metric:
						monitor_metric = val_ll.numpy()
						logger.info("Saving the best model (best policy_loss: {}) at epoch: {}".format(
							monitor_metric, epoch + 1))
...
def run(args):
	...
	expert_worker = RolloutWorker(
		exp_env, expert_policy, T=args.horizon, rollout_terminate=args.rollout_terminate, compute_c=True,
		exploit=True, noise_eps=0., random_eps=0., compute_Q=False, use_target_net=False, render=False)
	
	# ############################################# DATA TRAINING ############################################# #
	# Load Buffer to store expert data
	expert_buffer = ReplayBufferTf(get_buffer_shape(args), args.buffer_size, args.horizon, sample_c_aware_transitions)
	...
	if args.do_verify:
		...
		test_rollout_worker = RolloutWorker(
			...
			exploit=True, noise_eps=0., random_eps=0., compute_c=True, use_target_net=False, render=True)
