...
from her.transitions import sample_all_consecutive_transitions, sample_her_fwd_bkd_transitions
...
def run(args):
	...
	expert_worker = RolloutWorker(exp_env, expert_policy, T=args.horizon, rollout_terminate=args.rollout_terminate, is_expert=True, exploit=True, noise_eps=0., random_eps=0., compute_Q=False, use_target_net=False, render=False)
	...
	env_state_dir = os.path.join(args.dir_data, '{}_env_states_train'.format('two_obj' if args.two_object else 'single_obj'))
	...
	if not os.path.exists(train_data_path):
		...
		for i in range(num_train_episodes):
			...
			with open(path_to_init_state_dict.numpy(), 'wb') as handle:
				# init_state_dict = expert_worker.env.get_state_dict()  # Call it from outside since when run in non-eager mode, exp_stats store the init_state_dict generated during tracing. Else generate data in execution mode
	...
	env_state_dir = os.path.join(args.dir_data, '{}_env_states_val'.format('two_obj' if args.two_object else 'single_obj'))
	if not os.path.exists(env_state_dir):
		os.makedirs(env_state_dir)
