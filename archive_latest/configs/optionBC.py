...
def get_optionBC_args(log_dir):
	parser = argparse.ArgumentParser()
	
	parser.add_argument('--do_pretrain', type=bool, default=False,
						help='Whether to pretrain the Option policies')
	parser.add_argument('--do_train', type=bool, default=True,
						help='Whether to train the Goal/Skill Predictor [Assumes pretraining is done]')
	parser.add_argument('--do_adversarial_train', type=bool, default=True,
						help='Whether to train the Goal/Skill Predictor with Adversarial Training')
	...
	parser.add_argument('--num_options', type=int, default=3, help='Number of options to use')
	...
	parser.add_argument('--num_epochs', type=int, default=300, help='Recommended Use:- 1000')
	...
	parser.add_argument('--ae_loss_weight', type=float, default=1.0, help='[AAE] AutoEncoder Coefficient')
	parser.add_argument('--disc_loss_weight', type=float, default=1.0, help='[AAE] Discriminator Coefficient')
	parser.add_argument('--gen_loss_weight', type=float, default=1.0, help='[AAE] Generator/Encoder Coefficient')
	parser.add_argument('--grad_norm_clip', type=float, default=1.0, help='Gradient Norm Clip')
	parser.add_argument('--future_gamma', type=float, default=0.1,
						help='How far ahead into the future to predict the goals')
	...
	# log_dir = './logging/20220909-144802'
	...
	args.train_trans_style = 'options' if args.do_pretrain else 'random'
	args.val_trans_style = 'options' if args.do_pretrain else 'all'
	args.val_batch_size = 512 if args.do_pretrain else args.val_demos
	
	# Adversarial Training only if Training
	args.do_adversarial_train = args.do_adversarial_train and args.do_train
