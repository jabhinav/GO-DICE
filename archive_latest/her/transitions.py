...
def sample_goal_oriented_transitions(sample_style: str, future_offset_frac: float = 0.5,
	...
	def _sample_options_transitions(episodic_data, batch_size_in_transitions=None, num_options=3):
		...
		for i in range(num_options):
			...
			if obj_space_as_goal:
				future_ag = tf.numpy_function(get_obj_goal, [episodic_data['obj_identifiers'],
															 episodic_data['achieved_goals'],
															 future_indices], Tout=tf.float32)
			else:
	...
	def _sample_all_transitions(episodic_data, num_episodes):
		"""
	...
	if sample_style == 'random':
		...
		return _sample_options_transitions
	else:
