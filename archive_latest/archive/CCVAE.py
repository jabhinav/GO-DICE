...
from utils.sample import get_transn_loss, multi_sample_normal_np, kl_divergence_gaussian
from utils.plot import plot_vae_loss
from evaluation.eval import evaluate_model_discrete
from domains.stackBoxWorld import stackBoxWorld, _find
...
class Decoder(tf.keras.Model):
    def __init__(self, a_dim):
        ...
        self.fc4 = Dense(units=128, activation=tf.nn.relu)
        self.add = Add()
        self.action_out = Dense(units=a_dim, activation=tf.nn.softmax)
...
class CCVAE(tf.keras.Model):
    def __init__(self, z_dim, y_dim, a_dim):
...
class Agent:
    def __init__(self, s_dim, stack_s_dim, a_dim, z_dim, y_dim, num_samples, supervision, train_config):
    ...
    def load_model(self,  param_dir, model_id):
        ...
        # BUILD First
        _ = model.encoder(np.ones([1, self.train_config['w_size'] * 5, 5, 6]), np.ones([1, self.y_dim]))
        _ = model.decoder(np.ones([1, 5, 5, 6]), np.ones([1, self.z_dim]))
        _ = model.classifier(np.ones([1, self.z_dim]))
    ...
    def act(self, state, encode_z, action=None):
        action_prob = self.model.decoder(state, encode_z)
...
def run(env_name, exp_num=0, sup=0.1, fixed_transition=False):

    print("\n\n---------------------------------- Supervision {} : Exp {} ----------------------------------".format(sup, exp_num),
          file=open(file_txt_results_path, 'a'))
    ...
    y_dim = 6
    state_dim = (5, 5, 6)
    stack_state_dim = (train_config["w_size"]*5, 5, 6)
    action_dim = 4
...
if __name__ == "__main__":
    ...
    for perc_supervision in supervision_settings:
        run(env_name='StackBoxWorld', sup=perc_supervision, fixed_transition=True)
