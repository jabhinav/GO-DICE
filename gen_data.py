...
from utils.env import get_PnP_env, save_env_img
...
def run(args):
    ...
    while n < args.num_demos:
        ...
        if tf.math.equal(tf.argmax(_episode['successes'].numpy()[0]), 0) \
            ...
            # To save final state after each episode
            save_env_img(expert_worker.env, os.path.join(args.dir, 'Env_{}_{}.png'.format(data_type, n)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--horizon', type=int, default=200,
                        help='Set 150 for one_obj, 150 for two_obj and '
                             '200 for three_obj (stack) / 250 for three_obj (w self-correction)')
