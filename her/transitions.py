...
def get_segmented_transitions(c_specific_mask: np.ndarray, pick_segment_boundary_prob: float = 0.5):
	t_samples = []
	past_t = []
	future_t = []
	for rollout_id in range(c_specific_mask.shape[0]):
		...
		if len(t_segments):
			...
			# Select a time step randomly from the segment (Don't use t_segment[1:-1] as transition boundaries are imp.)
			t_curr = random.choice(t_segment)
			# With some probability, override the t_curr to be the first or last time step of the segment
			if np.random.uniform(0, 1) < pick_segment_boundary_prob:
			...
			# Determine the length of future segment and then the future offset (can be same as t_curr)
			...
			# Determine the length of past segment and then the past offset (can be same as t_curr)
...
def sample_transitions(sample_style: str, state_to_goal=None, num_options: int = None):
	...
	def sample_skills_random_transitions(episodic_data, batch_size_in_transitions=None,
										 future_in_segment=True, past_in_segment=False):
		"""
		...
		for i in range(num_options):
			...
			# Check if the episode has the option
			if not tf.reduce_any(t_mask_trg_c):
				option_transitions[i] = None
				continue
			...
			if future_in_segment:
				...
				future_offset = rdm_future_offset_frac * tf.cast((terminate_idxes - t_samples),
																 rdm_future_offset_frac.dtype)
			...
			if past_in_segment:
				...
				t_samples_init = tf.zeros_like(t_samples, dtype=episode_idxs.dtype)
			...
			option_transitions[i]['achieved_goals'] = state_to_goal(
	...
	def sample_random_transitions(episodic_data, batch_size_in_transitions=None):
		...
		transitions['achieved_goals'] = state_to_goal(states=tf.gather_nd(episodic_data['states'], indices=curr_indices),
													  obj_identifiers=None)
