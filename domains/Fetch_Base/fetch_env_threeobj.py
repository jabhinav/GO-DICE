...
MODEL_XML_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'asset', 'fetch', 'pick_and_place_threeobj.xml')
...
class FetchEnv(robot_env.RobotEnv):
	...
	def _sample_goal(self):
		if self.has_object:
			...
			object_qpos2 = self.sim.data.get_joint_qpos('object2:joint')[:3]
			...
			if not self.stacking:
				...
				# # ---------------- Set the third goal --------------------------------
				goal2 = self.initial_gripper_xpos[:3] + self.np_random.uniform(-self.target_range,
																			   self.target_range,
																			   size=3)
				goal2 += self.target_offset
				goal2[2] = self.height_offset
				
				# ----------- Cond 3: To make the third goal sufficiently far from first and second goals and objects ------------
