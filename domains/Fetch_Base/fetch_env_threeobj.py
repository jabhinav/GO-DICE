...
MODEL_XML_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'asset', 'fetch',
                              'pick_and_place_threeobj.xml')
...
class FetchEnv(MujocoRobotEnv):
    ...
    def _get_obs(self):
        ...
        # else:
        # 	achieved_goal = np.concatenate([np.squeeze(object1_pos.copy()),
        # 									np.squeeze(object2_pos.copy()),
        # 									np.squeeze(object3_pos.copy())])
        # obs = np.concatenate([
        ...
        if self.has_object:
            ...
            object3_pos = self._utils.get_site_xpos(self.model, self.data, "object2")
            ...
            object3_rot = rotations.mat2euler(
                self._utils.get_site_xmat(self.model, self.data, "object2")
            )
            ...
            object3_velp = (
                    self._utils.get_site_xvelp(self.model, self.data, "object2") * dt
            )
            object3_velr = (
                    self._utils.get_site_xvelr(self.model, self.data, "object2") * dt
            )
    ...
    def _render_callback(self):
        ...
        # self.sim.model.site_pos[site_id2] = self.goal[6:9] - sites_offset[0]
        # self.sim.forward()
        ...
        site_id2 = self._mujoco.mj_name2id(
            self.model, self._mujoco.mjtObj.mjOBJ_SITE, "target2"
        )
        self.model.site_pos[site_id] = self.goal[:3] - sites_offset[0]
        self.model.site_pos[site_id1] = self.goal[3:6] - sites_offset[0]
        self.model.site_pos[site_id2] = self.goal[6:9] - sites_offset[0]
        self._mujoco.mj_forward(self.model, self.data)
    ...
    def _reset_sim(self):
        ...
        #
        # 	# Update z position (we want to keep same height, height_offset was set in _env_setup)
        # 	object_qpos[2] = self.height_offset
        # 	object_qpos1[2] = self.height_offset
        # 	object_qpos2[2] = self.height_offset
        #
        # 	self.sim.data.set_joint_qpos('object0:joint', object_qpos)
        # 	self.sim.data.set_joint_qpos('object1:joint', object_qpos1)
        # 	self.sim.data.set_joint_qpos('object2:joint', object_qpos2)
        #
        # self.sim.forward()
        # return True
        ...
        if self.has_object:
            ...
            self._utils.set_joint_qpos(self.model, self.data, "object2:joint", object_qpos2)
    ...
    def _sample_goal(self):
        if self.has_object:
            ...
            object_qpos2 = self._utils.get_joint_qpos(self.model, self.data, "object2:joint")[:3]
            ...
            if not self.stacking:
                ...
                while np.linalg.norm(goal0 - goal1) < 0.1 or \
                        ...
                        np.linalg.norm(goal1 - object_qpos1) < 0.1 or \
                        np.linalg.norm(goal1 - object_qpos2) < 0.1:
                ...
                goal2 += self.target_offset
                goal2[2] = self.height_offset

                # ----------- Cond 3: To make the third goal sufficiently far from first, second goal and objects ------------
                while np.linalg.norm(goal0 - goal2) < 0.1 or \
                        np.linalg.norm(goal1 - goal2) < 0.1 or \
                        np.linalg.norm(goal2 - object_qpos) < 0.1 or \
                        np.linalg.norm(goal2 - object_qpos1) < 0.1 or \
                        np.linalg.norm(goal2 - object_qpos2) < 0.1:
                    # print("Cond3: resampling the goal2")
                    goal2 = self.initial_gripper_xpos[:3] + self.np_random.uniform(
                        -self.target_range, self.target_range, size=3
                    )
                    goal2 += self.target_offset
                ...
                # if target_in_the_air and with 0.5 selection prob, inc. z_pos of the goal2
                if self.target_in_the_air and self.np_random.uniform() < 0.5:
                    goal2[2] += self.np_random.uniform(0, 0.45)
    ...
    def _env_setup(self, initial_qpos):
        """
        Initial configuration of the environment. Done once at the initialization of environment. (not reset)
        """
        ...
        # if self.has_object:
        # 	# Set height_offset (for some reason the object0 is not at the height of the object1/2). This is happening
        # 	# after sim.step() in the above for loop. In two object case, it is not happening. So, we are using the
        # 	# object1's height as the height_offset instead of object0's height.
        ...
        if self.has_object:
            # Set height_offset (for some reason the object0 is not at the height of the object1,2). This is happening
            # after mj_step.step() in the above for loop. In two object case, it is not happening. So, we are using the
            # object1's height as the height_offset instead of object0's height.
            self.height_offset = self._utils.get_site_xpos(self.model, self.data, "object1")[2]
...
class FetchPickAndPlaceEnv(FetchEnv, EzPickle):
    def __init__(self, reward_type='sparse', stacking=True, first_in_place=False, distance_threshold=0.01):
